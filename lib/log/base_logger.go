...
func (l *baseEncoder) Reset() {
	l.Encoder.Reset()
	l.buf.Reset()
}

var baseEncoderPool = sync.Pool{
	New: func() interface{} {
		var enc baseEncoder
...
func NewBaseLogger(w io.Writer) kitlog.Logger {
	return &baseLogger{w}
}

func (l baseLogger) Log(keyvals ...interface{}) error {
	enc := baseEncoderPool.Get().(*baseEncoder)
	enc.Reset()
	defer baseEncoderPool.Put(enc)
